...
- 事業名を変更：メディア事業→PR事業、セールス事業→販売事業、プロダクト事業→自社ブランド事業。自社ブランド事業のラベルは「COMING SOON」（トップ・BUSINESS・会社概要・metaに反映）
...
- [ ] Lucaffe：取扱区分の正式名称（正規代理店・正規販売店など）を確認し、カードとページの表記を統一
      （「月200万杯」「業界最小」の類の数量・最上級表記は掲載しない［2026-08-19 確定］。数値は横幅15cm・35mm等の素の事実のみ）
- [ ] Lucaffe：定期お届け便の条件（貸与元＝テクタイト ルカフェ株式会社、無料修理の期間と起点、最低利用6回）を最終確認
- [x] Lucaffe：画像7点（ステンレス・ゴールド・ダイヤモンドパターン）を受領し、ヒーロー・仕上げ3種ギャラリー・暮らしのシーン・カードに配置済み（2026-08-19）
- [ ] 画像のWebP変換（現状はJPEG。この環境に変換ツールがないため未実施）
- [ ] テクタイトから正式な取扱機種リスト・価格表（メーカー希望小売価格）を取り寄せ、JSONと照合
      （製品写真は2026-08-19に8機種すべて実写を配置済み。タイポグラフィプレートは解消）
