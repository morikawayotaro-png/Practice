...
- 経営陣ポートレート（A-3）：別途用意される写真に差し替え（`/about/` はプレースホルダーのまま）
- Shot Navi 製品プレート写真：テクタイト公式の製品画像に差し替え（現在プレースホルダー。納品済みイメージカットは実機を示さないため使用しない）
- SMEG公式素材：ギャラリー下段・コラボレーション枠に配置（背景をアイボリー#F5F1E8に置き換え、彩度を落とす。第9章）
- OGP画像（`assets/ogp.png`）の制作
- 送信ドメイン認証（SPF・DKIM・DMARC）、`www` 有無の301統一とhttps強制（サーバー設定）
- プライバシーポリシーの専門家確認
